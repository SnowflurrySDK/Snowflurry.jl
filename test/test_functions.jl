...
function test_inverse(gate::Gate)
    ...
    return (
        get_operator(get_gate_symbol(gate)) * get_operator(get_gate_symbol(inverse_gate)) ≈
        eye(2^target_count)
    )
end
...
function make_array(
    ...
    values::Vector{<:Number} = [1.0, 2.0, 3.0, 4.0],
)
...
function test_operator_implementation(
    ...
    @testset "$test_label" begin
        ...
        @test op[first_row, first_col] === ComplexF32(1.0)
    ...
    @testset "$test_label" begin
        ...
        Snowflurry.apply_operator!(ψ_1, op, [v for v = 1:get_num_qubits(op)])
