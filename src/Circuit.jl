...
export Circuit, pushGate!, popGate!, simulate, simulateShots, submitCircuit, createJobRequest
...
function submitCircuit(circuit::Circuit; owner::String, shots::Int, host::String="localhost:50051")
    client = AnyonClients.CircuitAPIClient(host)
    request = createJobRequest(circuit, owner=owner, shots=shots)
    try
...
function createJobRequest(circuit::Circuit; owner::String, shots::Int)
    pipeline = []
    for step in circuit.pipeline
    ...
    request = AnyonClients.SubmitJobRequest(owner=owner, shots_count=shots, circuit=circuit_api)
